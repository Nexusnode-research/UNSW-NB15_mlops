...
\author{
    \href{https://orcid.org/0009-0002-9752-0688}{\includegraphics[scale=0.06]{orcid.pdf}\hspace{1mm}Vince Mbanze}\\
...
\midrule
This work (2025) & UNSW-NB15 fixed splits; stratified validation; reproducible preprocessing & LR, MLP, RF, XGBoost & ROC-AUC, Precision, Recall, F1, FPR, Confusion matrix & ONNX export, FastAPI service, CI/CD smoke tests, threshold governance & -\\
\addlinespace
...
Feature processing follows the repository’s \texttt{engineer.py}:
\begin{itemize}
...
\end{itemize}
\textbf{Parity checks.} \texttt{tests/test\_bundle.py} asserts schema presence, threshold persistence, and ONNX loadability. \texttt{onnx\_smoke.py} verifies probabilities/labels against the saved scaler and feature order.
...
\textbf{Containerisation and orchestration.} Slim Docker images are built for the API and UI. Kustomize overlays manage environment-specific Kubernetes manifests (replica counts, resource requests/limits). Since all API endpoints are Bearer-authenticated, probes are configured to supply the authorization header; alternatively, \texttt{/health} can be exempted. This ensures liveness/readiness checks reflect true production posture. A CPU-based Horizontal Pod Autoscaler (HPA) is configured for the API to scale replicas under load. These patterns align with recommended CI/CD automation for Kubernetes workloads \cite{shrestha2024}.
...
\textbf{CI/CD.} A GitHub Actions pipeline automates integration and deployment. It builds the API/UI images, pushes them to ECR, and executes containerised smoke tests (including an automated API Postman collection) against a disposable instance. On success, the pipeline applies the overlay to EKS to roll out the new image. Rollouts are gated by Kubernetes readiness and deployment status, with image provenance preserved by the commit-SHA tag used at deploy time. This automated loop enables reproducible, secure, and fast promotion from development to production, consistent with modern DevOps guidelines \cite{sinde2022}.
...
\begin{figure}[t]
  ...
  The UI is public behind a Service \texttt{LoadBalancer}; the API is internal behind a Service \texttt{ClusterIP}. The Horizontal Pod Autoscaler (HPA) scales API replicas. Container images are built and pushed by GitHub Actions and deployed to EKS via OIDC assume-role (IRSA); images are stored in Amazon ECR with both tags (e.g., \texttt{latest}) and digests (\texttt{image@sha256}). Secrets flow from AWS Secrets Manager through the External Secrets controller into Kubernetes Secrets and are mounted by pods. The API pulls model bundles and logs metrics to the MLflow Registry.}
  \label{fig:eks-arch}
\end{figure}
...
\textbf{Auth on health.} Guarding \texttt{/health} with the same Bearer policy as production avoids “probe backdoors,” but complicates liveness/readiness. Probes must either (a) authenticate (e.g., \texttt{exec} probe with header) or (b) hit an exempt \texttt{/health} that returns only bundle/process hashes. Current manifests use unauthenticated HTTP probes, which will fail against the auth-guarded endpoint—this should be revised.
...
\begin{itemize}
    \item \textbf{Contract:} schema-mismatch rate and 400s at \texttt{/predict(\_proba)}, scaler/bundle load failures, bundle-hash drift.
